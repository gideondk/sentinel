...
object SentinelServer {
  ...
  def apply[Evt, Cmd, Context <: PipelineContext](serverPort: Int, handler: Evt ⇒ Future[Cmd], description: String = "Sentinel Server")(pipelineCtx: ⇒ Context, stages: PipelineStage[Context, Cmd, ByteString, Evt, ByteString], ackCount: Int = 10)(implicit system: ActorSystem) = {
    system.actorOf(Props(new SentinelServer(serverPort, description, new SentinelServerWorker[Cmd, Evt, Context](pipelineCtx, stages, handler, description + " Worker", ackCount))))
  }
  ...
  //                                                          handler: Evt ⇒ Future[Cmd], description: String = "Sentinel Server")(pipelineCtx: ⇒ Context, stages: PipelineStage[Context, Cmd, ByteString, Evt, ByteString], ackCount: Int = 10)(implicit system: ActorSystem) =
  //    apply(serverPort, RandomRouter(numberOfWorkers), handler, description)(pipelineCtx, stages, ackCount)
  //
  //  /** Creates a new SentinelServer
  //   *
  //   *  @tparam Evt event type (type of requests send to server)
  ...
  //                                                              handler: Evt ⇒ Future[Cmd], description: String = "Sentinel Server")(pipelineCtx: ⇒ Context, stages: PipelineStage[Context, Cmd, ByteString, Evt, ByteString], ackCount: Int = 10)(implicit system: ActorSystem) =
  //    apply(serverPort, RoundRobinRouter(numberOfWorkers), handler, description)(pipelineCtx, stages, ackCount)
