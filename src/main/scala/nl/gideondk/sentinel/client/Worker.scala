...
class SentinelClientWorker[Cmd, Evt, Context <: PipelineContext](pipelineCtx: ⇒ Context, stages: PipelineStage[Context, Cmd, ByteString, Evt, ByteString],
  ...
  val ioActor = context.actorOf(Props(new SentinelClientIOWorker(workerDescription + "-IO", writeAck)).withDispatcher("nl.gideondk.sentinel.sentinel-client-worker-dispatcher"))
