...
object SimpleServerHandler extends DefaultSimpleMessageHandler {
  ...
  override def process = super.process orElse {
    case SimpleCommand(PING_PONG, payload) ⇒ ProducerAction.Signal { x: SimpleCommand ⇒ Thread.sleep(100); Future(SimpleReply("PONG")) }
    ...
    case SimpleCommand(GENERATE_ERROR, payload) ⇒ ProducerAction.ProduceStream { x: SimpleCommand ⇒
      // Future((Enumerator(List.range(0, count): _*) &> Enumeratee.map(x ⇒ SimpleStreamChunk(x.toString))) >>> Enumerator(SimpleStreamChunk("")))
      Future(Enumerator(SimpleError("")))
    }
